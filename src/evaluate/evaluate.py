...
class TFEvaluator():
    def __init__(self, test_prompt_id, dev_dataset, test_dataset, save_path="checkpoints", model_name="best_model.h5", log_file="logs/log.txt", mode=""):
        ...
        if not (self.mode == "prompt_tuning"):
    ...
    def evaluate(self, model, epoch, loss_func, print_info=True, save_info=True):
        self.current_epoch = epoch
        
        dev_pred = model.predict(self.dev_features_list, batch_size=2)
        dev_loss = loss_func(self.Y_dev, dev_pred)
        ...
        if not (self.mode == "prompt_tuning"):
            ...
            model.save_weights(self.model_path.replace("best_model", "current_model"), save_format="h5")
            ...
            model.save_weights(self.model_path.replace("best_model", "current_model"), save_format="h5") # TODO: Currently hard code
            if dev_loss < self.best_dev_loss:
                self.best_dev_loss = dev_loss
                self.best_dev_epoch = epoch
                model.save_weights(self.model_path, save_format="h5")
            print("Best dev epoch: ", self.best_dev_epoch)
            print("Best dev loss: ", self.best_dev_loss)
